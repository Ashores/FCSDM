...
def add_clip_support_to_memory_cluster():
    ...
    def init_sample_metadata(self):
        self.sample_metadata = {}  # 存储样本的元数据信息
    ...
    def new_method(self, sample1, sample2):
        if self.distance_type == "CLIP":
            distance, _, _ = self.calculate_clip_distance(sample1, sample2)
            return distance
        else:
    ...
    def add_sample_with_causal_constraint(self, x1):
        """简化版本的添加样本方法，使用距离约束但不处理复杂元数据"""
        # 首次使用时初始化结构
        if not hasattr(self, 'causal_graph'):
            self.init_causal_graph()
        if not hasattr(self, 'sample_metadata'):
            self.init_sample_metadata()
        
        # 定义距离阈值
        distance_threshold = 0.2  # 添加距离阈值定义
        ...
        # 计算距离 - 正确解包元组
        distance, _, _ = self.calculate_clip_distance(x1, self.centreSample)
...
def add_soft_clustering_support():
    ...
    def add_data_batch_soft_clustering(self, x1):
        ...
        """
        # 统一距离阈值
        add_threshold = 0.5  # 距离小于此值添加到多个簇中
        new_cluster_threshold = 0.2  # 距离大于此值创建新簇
        ...
        for i in range(n):
            ...
            for j in range(np.shape(self.MemoryClusterArr)[0]):
                ...
                # 计算距离
                dis1, _, _ = m1.calculate_clip_distance(m1.centreSample, data1)
...
def optimize_memory_framework():
    ...
    def optimized_add_data_batch_soft_clustering(self, x1):
        ...
        实现逻辑：
        1. 每个样本都加入到与其最相似的簇
        2. 如果样本与多个簇的距离都小于0.15，则加入所有这些簇
        3. 如果样本与所有簇的距离都大于0.2，则创建新簇
        ...
        """
        # 统一的距离阈值
        add_threshold = 0.15  # 距离小于此值添加到多个簇中
        new_cluster_threshold = 0.2  # 距离大于此值创建新簇
...
def main():
    ...
    myThirdName = "results/ScoreCurve_SVHN" + "_" + str(threshold) + ".txt"
    ...
    myThirdName = "results/SVHN_Class"  + "_" + str(threshold) + ".txt"
    #myThirdName = "results/Diffusion_Forgetting_RecoLoss_FirstTaskLearning.txt"
    f = open(myThirdName, "w", encoding="utf-8")
    for i in range(np.shape(arr1)[0]):
        f.writelines(arr1[i])
    ...
    myThirdName = "results/SVHN_Component" + "_" + str(threshold) + ".txt"
    #myThirdName = "results/Diffusion_Forgetting_RecoLoss_FirstTaskLearning.txt"
    f = open(myThirdName, "w", encoding="utf-8")
    for i in range(np.shape(arr1)[0]):
        f.writelines(arr1[i])
