...
# 全局阈值参数
ADD_THRESHOLD = float(os.getenv('ADD_THRESHOLD', '0.1'))
NEW_CLUSTER_THRESHOLD = float(os.getenv('NEW_CLUSTER_THRESHOLD', '0.15'))
...
def add_clip_support_to_memory_cluster():
    ...
    def init_sample_metadata(self):
        """初始化样本元数据存储"""
        self.sample_metadata = {}
    
    MemoryCluster.init_causal_graph = init_causal_graph
    MemoryCluster.init_sample_metadata = init_sample_metadata
    ...
    def calculate_clip_distance(self, sample1, sample2):
        ...
        if isinstance(sample1, np.ndarray):
            sample1_tensor = torch.from_numpy(sample1).to(device)
        else:
            sample1_tensor = sample1.to(device)
            
        if isinstance(sample2, np.ndarray):
            sample2_tensor = torch.from_numpy(sample2).to(device)
        else:
            sample2_tensor = sample2.to(device)
        
        # 提取CLIP特征
        feature1 = extract_clip_features_with_cache(sample1_tensor.unsqueeze(0), device)
        feature2 = extract_clip_features_with_cache(sample2_tensor.unsqueeze(0), device)
    ...
    def add_sample_with_causal_constraint(self, x1):
        # First time use, initialize structures
        if not hasattr(self, 'causal_graph'):
            self.init_causal_graph()
        ...
        if len(self.arr) == 0:
            ...
            # Save metadata with max similarity
            sample_hash = hash(x1.tobytes())
            self.sample_metadata[sample_hash] = {
                'timestamp': time.time(),
                'initial_similarity': 1.0  # Perfect similarity for first sample
            }
            return True
        
        # Calculate distance once
        if isinstance(x1, np.ndarray):
            sample_tensor = torch.from_numpy(x1).to(self.device)
        else:
            sample_tensor = x1.to(self.device)
        feature = extract_clip_features_with_cache(sample_tensor.unsqueeze(0), self.device)
        
        if isinstance(self.centreSample, np.ndarray):
            center_tensor = torch.from_numpy(self.centreSample).to(self.device)
        else:
            center_tensor = self.centreSample.to(self.device)
        center_feature = extract_clip_features_with_cache(center_tensor.unsqueeze(0), self.device)
        
        # Get similarity (already calculated for decision making)
        similarity = calculate_causal_similarity(feature, center_feature, self.causal_graph)
        distance = 1.0 - similarity
        
        # Add the sample and save metadata
        if distance < distance_threshold:
            self.AddSingleSample(x1)
            sample_hash = hash(x1.tobytes())
            self.sample_metadata[sample_hash] = {
                'timestamp': time.time(),
                'feature': feature,  # Save feature for reuse
                'initial_similarity': similarity  # Keep original similarity
            }
            return True
        else:
            # Still add but mark as less relevant
            self.AddSingleSample(x1)
            sample_hash = hash(x1.tobytes())
            self.sample_metadata[sample_hash] = {
                'timestamp': time.time(),
...
def add_soft_clustering_support():
    ...
    def add_data_batch_soft_clustering(self, x1):
        ...
        实现逻辑：
        1. 每个样本都加入到与其最相似的簇
        2. 如果样本与多个簇的距离都小于0.1，则加入所有这些簇
        3. 如果样本与所有簇的距离都大于0.15，则创建新簇
        """
        # 统一距离阈值
        add_threshold = 0.1  # 距离小于此值添加到多个簇中
        new_cluster_threshold = 0.15  # 距离大于此值创建新簇
...
def optimize_memory_framework():
    ...
    def optimized_add_data_batch_soft_clustering(self, x1):
        ...
        实现逻辑：
        1. 每个样本都加入到与其最相似的簇
        2. 如果样本与多个簇的距离都小于add_threshold，则加入所有这些簇
        3. 如果样本与所有簇的距离都大于new_cluster_threshold，则创建新簇
        ...
        """
        # 使用从命令行传入的阈值参数
        add_threshold = self.add_threshold if hasattr(self, 'add_threshold') else 0.1
        new_cluster_threshold = self.new_cluster_threshold if hasattr(self, 'new_cluster_threshold') else 0.15
        ...
        def update_center_features():
            ...
            for j, cluster in enumerate(self.MemoryClusterArr):
                ...
                if isinstance(cluster.centreSample, np.ndarray):
                    ...
                    center_sample = cluster.centreSample.to(self.device)
                center_features[j] = extract_clip_features_with_cache(center_sample.unsqueeze(0), self.device)
            ...
            for j in range(np.shape(self.MemoryClusterArr)[0]):
                ...
                if cache_key not in batch_features:
                    ...
                    if isinstance(data1, np.ndarray):
                        ...
                        data_tensor = data1.to(self.device)
                    batch_features[cache_key] = extract_clip_features_with_cache(data_tensor.unsqueeze(0), self.device)
...
def main():
    ...
    # 设置阈值参数
    TSFramework.add_threshold = ADD_THRESHOLD
    TSFramework.new_cluster_threshold = NEW_CLUSTER_THRESHOLD
    ...
    for i, cluster in enumerate(TSFramework.MemoryClusterArr):
        ...
        if hasattr(cluster, 'causal_graph'):
            ...
            for sample in cluster.arr[:min(10, len(cluster.arr))]:  # 只取前10个样本分析
                ...
                feature = extract_clip_features_with_cache(sample_tensor.unsqueeze(0), device)
