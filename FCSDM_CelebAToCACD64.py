...
def main():
    ...
    trainX1 = np.concatenate((trainX1,trainX2),0)
        ...
        # 复制数据为重复的两个 - 直接使用张量操作
    if isinstance(trainX1, torch.Tensor):
        trainX1 = torch.cat([trainX1, trainX1], dim=0)
    else:
        trainX1 = np.concatenate([trainX1, trainX1], axis=0)
    print("Duplicated data shape:", trainX1.shape)
    ...
    TSFramework.MaxMemoryCluster = 40
    ...
    for step in range(TSFramework.currentTraningTime,totalTrainingTime):
        ...
        batch = LoadFFHQFromPath(batchFiles)
        print(f"Initial batch shape: {batch.shape}")
        print(f"Initial memoryBuffer shape: {memoryBuffer.shape}")
        # 调整batch的维度顺序以匹配memoryBuffer
        # 如果memoryBuffer是[batch, height, width, channels]格式，需要调整batch
        if len(memoryBuffer.shape) == 4 and len(batch.shape) == 4:
            if memoryBuffer.shape[1] != batch.shape[1]:  # 检查第二个维度是否不匹配
                # 将batch从[batch, channels, height, width]转换为[batch, height, width, channels]
